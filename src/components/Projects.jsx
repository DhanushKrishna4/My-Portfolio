      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div key={index} className="bg-white/5 p-6 rounded-xl border border-red-900/30 hover:border-red-600/50 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(153,27,27,0.2)] group backdrop-blur-sm">
              <h3 className="text-xl font-bold text-slate-100 mb-2 group-hover:text-red-500 transition-colors">{project.title}</h3>
              ...
              <a href={project.link} className="inline-flex items-center text-sm font-semibold text-red-600 hover:text-red-400">View Code</a>
            </div>
          ))}
        </div>
      </div>
